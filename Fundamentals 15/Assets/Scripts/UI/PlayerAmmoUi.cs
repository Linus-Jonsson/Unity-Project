...
public class PlayerAmmoUi : MonoBehaviour {
	...
	void Start() {
		...
		for (int i = 0; i < playerShoot.maxProjectiles; i++) {
			Image backRoundsSpruta = Instantiate(backroundSpruta,
				GetComponent<RectTransform>().position + Vector3.right * xDist * i,
				image.rectTransform.rotation);
			backRoundsSpruta.rectTransform.SetParent(GetComponent<RectTransform>());
		}

		for (int i = 0; i < playerShoot.maxProjectiles; i++) {
			Image spruta = Instantiate(image,
				GetComponent<RectTransform>().position + Vector3.right * xDist * i,
				image.rectTransform.rotation);
